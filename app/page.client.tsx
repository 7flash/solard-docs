  ...
  const cells = Array.from({ length: 16 }, () => {
    ...
    if (roll < 0.42) {
      return {
        kind: "up",
        action: "▲ BUY",
        amount: `${randomBetween(0.05, 3).toFixed(2)} SOL`,
        token: pick(NAMES),
        detail: `mc $${marketCap()}`,
      };
    }
    ...
    <>
      {[...cells, ...cells].map((cell, index) => (
        <span data-index={index}>
          <b className={cell.kind}>{cell.action}</b>
...
function FeeCalculator({
  ...
  const yearlyFee = Math.round(value * 1000 * 0.01 * 12).toLocaleString(
    "en-US",
  );
  return (
    <>
      ...
      />
      <div className="out">
        at <b>{formatVolume(value)}</b>/mo, a 1% bot costs <b>${yearlyFee}</b>
        /yr · <span className="grn">solard: $0</span>
      </div>
    </>
  );
}
...
function setupCopyButtons() {
  ...
  document
    .querySelectorAll<HTMLButtonElement>(".cmd button")
    .forEach((button) => {
      const click = async () => {
...
function setupBootTerminal(reduceMotion: boolean) {
  ...
  const workers = [
    ...
    ([name, status]) =>
      `<span class="k">▸</span> ${name} <span class="k">${".".repeat(Math.max(2, 30 - name.length - status.length))}</span> <span class="ok">${status}</span>`,
  );
  const gate =
    '<span class="k">live trades:</span> <span class="ok">disabled</span> <span class="k">(SOLARD_ENABLE_LIVE_TRADES=0)</span>';
  const url =
    '<span class="k">console:</span> <span class="u">http://localhost:3000</span>';
  const timestamp = () =>
  ...
  const feedLine = () => {
    ...
    const roll = Math.random();
    if (roll < 0.14)
      return `<span class="k">${timestamp()}</span> <span class="y">＋ create</span> $${token} <span class="k">curve 0% · pump</span>`;
    if (roll < 0.6)
      return `<span class="k">${timestamp()}</span> <span class="ok">▲ buy</span> ${randomBetween(0.05, 2.5).toFixed(2)} SOL $${token} <span class="k">mc $${marketCap()}</span>`;
    if (roll < 0.94)
      return `<span class="k">${timestamp()}</span> <span class="bad">▼ sell</span> ${randomBetween(0.05, 2.5).toFixed(2)} SOL $${token} <span class="k">mc $${marketCap()}</span>`;
...
function setupObservers() {
  ...
  const links = [
    ...document.querySelectorAll<HTMLAnchorElement>(".nav-links a[data-s]"),
  ];
  const sections = links
    .map((link) => document.getElementById(link.dataset.s ?? ""))
...
function setupDistributionChart() {
  ...
  for (let index = 0; index < lineCount; index += 1) {
    const roll = seeded();
    const category: WalletLine["category"] =
      roll < 0.06
        ? "w"
        : roll < 0.24
          ? "a"
    ...
    if (category === "w") {
      ...
      for (let time = 0; time < timePoints; time += 1) {
        value = clamp(value + (seeded() - 0.48) * 0.02, 0.9, cap);
        values[time] = value;
      }
    } else if (category === "a") {
      ...
      for (let time = 0; time < timePoints; time += 1) {
        if (time < start) values[time] = 0;
          ...
          holding = clamp(holding + 0.01 + seeded() * 0.012, 0, cap);
          values[time] = holding;
        }
      }
    } else {
      value = seeded() * 0.3;
      for (let time = 0; time < timePoints; time += 1) {
        value = clamp(value + (seeded() - 0.5) * 0.03, 0, 0.5);
        values[time] = value;
      }
    }
    const alphabet =
    ...
    const address = Array.from(
      { length: 4 },
      () => alphabet[(seeded() * alphabet.length) | 0],
    ).join("");
    wallets.push({
      values,
  ...
  const draw = () => {
    ...
    wallets.forEach((wallet, index) => {
      if (index === hot) return;
      context.globalAlpha =
        hot >= 0 ? 0.12 : wallet.category === "c" ? 0.3 : 0.75;
      context.strokeStyle = wallet.color;
      context.lineWidth = 1;
      context.beginPath();
      for (let time = 0; time < timePoints; time += 1) {
        const x = xAt(time);
        const y = yAt(wallet.values[time]);
  ...
  const nearest = (mouseX: number, mouseY: number) => {
    const time = clamp(
      Math.round((mouseX / width) * (timePoints - 1)),
      0,
      timePoints - 1,
    );
    let best = -1;
