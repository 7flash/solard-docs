function setupCopyButtons() {
  ...
  document
    .querySelectorAll<HTMLButtonElement>(
      ".cmd button[data-copy], .v4-command button[data-copy]",
    )
    .forEach((button) => {
      const click = async () => {
        try {
...
function setupObservers() {
  ...
  const links = [
    ...document.querySelectorAll<HTMLAnchorElement>(
      ".v4-nav-links a[data-s], .v3-nav-links a[data-s], .nav-links a[data-s]",
    ),
  ];
...
export default function mount() {
  ...
  const cleanup = () => {
    ...
    if (window.__solardLandingCleanup__ === cleanup)
      delete window.__solardLandingCleanup__;
  };
