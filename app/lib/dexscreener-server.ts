  ...
  try {
    const response = await fetch(url, {
      headers: {
        ...
        "User-Agent": "solard-tradjs-market-view/1.2",
      },
      signal: controller.signal,
    });
    if (!response.ok)
      throw new Error(`DEX Screener returned HTTP ${response.status}`);
...
export function normalizeDexPair(pair: RawPair): DexMarket | null {
  ...
  return {
    ...
    baseToken: {
      ...
      name: String(
        pair.baseToken.name ?? pair.baseToken.symbol ?? "Unknown token",
      ),
    ...
    quoteToken: {
      ...
      name: String(
        pair.quoteToken?.name ?? pair.quoteToken?.symbol ?? "Unknown quote",
      ),
    ...
    imageUrl:
      typeof pair.info?.imageUrl === "string" ? pair.info.imageUrl : null,
...
export function selectBestDexPairs(rawPairs: RawPair[], limit: number) {
  ...
  for (const rawPair of rawPairs) {
    ...
    const existingScore = existing
      ? existing.liquidityUsd * 10 + existing.windows.h24.volume
      : -1;
    if (!existing || score > existingScore)
      bestByToken.set(market.baseToken.address, market);
  }
  return [...bestByToken.values()]
    .sort(
      (a, b) =>
        b.boostsActive - a.boostsActive ||
        b.windows.h24.volume - a.windows.h24.volume ||
...
export async function pairsForDexAddresses(addresses: string[]) {
  ...
  return asArray<RawPair>(
    await fetchDexJson(
      `tokens/v1/solana/${addresses.map(encodeURIComponent).join(",")}`,
    ),
  );
}
